...
def train(args, unmix, device, train_sampler, optimizer):
	...
	for x, y in pbar:
		...
		mix_onsets = torch.zeros((X.shape[0], X.shape[1], 1))
		for i in range(X.shape[1]):
			mix_onsets[:,i] = spectral_flux_vectorised_torch(magStft=X_avg[:,i], device=device)
		mix_onsets = mix_onsets.to(device)
		Y_hat = unmix(x, mix_onsets)
		Y = unmix.transform(y)
...
# =============================================================================
	...
	return losses.avg , mse_losses.avg, bce_sf_losses.avg, precision_values.avg, recall_values.avg, f_score_values.avg
...
def main():
    ...
    #if (args.mtl_conv==1):
    #print("Using convolutional MTL model")
    unmix = model_mtl.OpenUnmix_mtl_mul(
    ...
    for epoch in t:
        ...
        train_loss, train_mse_loss, train_bce_sf_loss, train_precision, train_recall, train_f_score = train(args, unmix, device, train_sampler, optimizer)
        ...
        valid_loss, valid_mse_loss, valid_bce_sf_loss, valid_precision, valid_recall, valid_f_score = valid(args, unmix, device, valid_sampler, j, target_path)
